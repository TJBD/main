...
def create_DATA_INFO(connection):
    # 从JSON文件读取表结构
    with open("datareceiver/DATA_INFO.json", "r") as file:
        table_structure = json.load(file)
    # 构建SQL建表语句
    create_table_sql = "CREATE TABLE IF NOT EXISTS data_info ("  # 创建数据信息表
    create_table_sql += "id INT AUTO_INCREMENT PRIMARY KEY, "  # 添加自增主键列
    for column, attrs in table_structure["DATA_INFO"].items():
    ...
    create_table(connection, create_table_sql, "data_info")
...
def create_TEV_PRPD(connection):
    # 从JSON文件读取表结构
    with open("datareceiver/TEV.json", "r") as file:
        table_structure = json.load(file)["TEV_PRPD"]
    # 构建SQL建表语句
    create_table_sql = "CREATE TABLE IF NOT EXISTS TEV_prpd_info ("
    create_table_sql += "id INT AUTO_INCREMENT PRIMARY KEY, "  # 添加自增主键列
    create_table_sql += "file_name VARCHAR(50), "  # 添加file_name列
    for column, attrs in table_structure["Info"].items():
        column_type = attrs["type"]
    ...
    create_table_sql += "date_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
    create_table_sql += ");"
    create_table(connection, create_table_sql, "TEV_prpd_info")


def create_TEV_PRPS(connection):
    # 从JSON文件读取表结构
    with open("datareceiver/TEV.json", "r") as file:
        table_structure = json.load(file)["TEV_PRPS"]
    # 构建SQL建表语句
    create_table_sql = "CREATE TABLE IF NOT EXISTS TEV_prps_info ("
    create_table_sql += "id INT AUTO_INCREMENT PRIMARY KEY, "  # 添加自增主键列
    create_table_sql += "file_name VARCHAR(50), "  # 添加file_name列
    for column, attrs in table_structure["Info"].items():
        column_type = attrs["type"]
    ...
    create_table_sql += "date_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
    create_table_sql += ");"
    create_table(connection, create_table_sql, "TEV_prps_info")
...
def main():
    ...
    create_DATA_INFO(connection=connection)  #创建数据信息表
    ...
    create_TEV_PRPD(connection=connection)    #创建TEV_PRPD信息表
    create_TEV_PRPS(connection=connection)    #创建TEV_PRPS信息表
